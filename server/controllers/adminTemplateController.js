...
export const getAllTemplatesAdmin = async (req, res) => {
  try {
    ...
    res.json({ success: true, stats, templates: processedTemplates });
  } catch (error) {
...
export const createTemplateAdmin = async (req, res) => {
  try {
    ...
    if (existingTemplate) {
      return res.status(400).json({ success: false, message: `Template "${trimmedName}" already exists` });
    }
    ...
    res.status(201).json({ success: true, message: 'Template created successfully', template: processed });
  } catch (error) {
...
export const updateTemplateAdmin = async (req, res) => {
  try {
    ...
    if (name) {
      const existingTemplate = await Template.findOne({ name: name.trim(), _id: { $ne: templateId } });
      if (existingTemplate) {
        return res.status(400).json({ success: false, message: 'Template name already in use' });
      }
    }
    ...
    const template = await Template.findByIdAndUpdate(templateId, updateData, { new: true, runValidators: true }).lean();
    if (!template) {
      return res.status(404).json({ success: false, message: 'Template not found' });
    }
