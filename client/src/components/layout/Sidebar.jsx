...
import {
  ...
  Database, ChevronLeft, ChevronRight, Terminal, Key, BarChart3, FileCode2
} from 'lucide-react'
...
const SidebarContainer = styled.aside`
  ...
  width: ${props => props.$collapsed ? '72px' : '270px'};
  overflow: hidden;
  flex-shrink: 0;
...
const LogoMark = styled.div`
  ...
  flex-shrink: 0;
  svg { width: 15px; height: 15px; color: white; }
`
...
const CollapseBtn = styled.button`
  ...
  color: #3b82f6; flex-shrink: 0; transition: background 0.15s;
...
const Divider = styled.div`
  height: 1px; background: rgba(59,130,246,0.08); margin: 0.5rem 0.25rem;
`
...
const Trigger = styled.button`
  ...
  border-radius: 10px; cursor: pointer; transition: background 0.15s; text-align: left;
  &:hover { background: #eff6ff; }
  ${props => props.$collapsed && `justify-content: center; padding: 0.65rem;`}
`
...
const SubLink = styled(NavLink)`
  ...
  padding: 0.42rem 0.625rem; border-radius: 8px; text-decoration: none;
  font-size: 0.825rem; font-weight: 400; color: #64748b;
  letter-spacing: 0.01em; transition: background 0.13s, color 0.13s; white-space: nowrap;
  &::before {
...
const StandaloneLink = styled(NavLink)`
  ...
  color: #334155; font-size: 0.875rem; font-weight: 400; letter-spacing: 0.01em;
  transition: background 0.15s, color 0.15s; white-space: nowrap;
  ${props => props.$collapsed && `justify-content: center; padding: 0.65rem;`}

  svg { width: 17px; height: 17px; color: #64748b; flex-shrink: 0; transition: color 0.15s; }
  span { opacity: ${props => props.$collapsed ? 0 : 1}; transition: opacity 0.2s ease; }
...
const UserName = styled.span`
  flex: 1; font-size: 0.85rem; font-weight: 500; color: #0f172a;
  white-space: nowrap; overflow: hidden; text-overflow: ellipsis;
  opacity: ${props => props.$hidden ? 0 : 1}; transition: opacity 0.2s ease;
`
...
const Sidebar = ({ collapsed, onToggle }) => {
  ...
  const renderSection = (section) => (
    <div key={section.key}>
      <Trigger $open={openKey === section.key} $collapsed={collapsed} onClick={() => toggle(section.key)}>
        <TriggerIcon $open={openKey === section.key}><section.icon /></TriggerIcon>
        <TriggerLabel $open={openKey === section.key} $collapsed={collapsed}>{section.label}</TriggerLabel>
        ...
        <SubMenu $open={openKey === section.key} $height={section.height}>
          <SubInner>
            {section.links.map(({ name, path }) => (
              <SubLink key={path} to={path}>{name}</SubLink>
            ))}
          </SubInner>
        </SubMenu>
      )}
    ...
    <SidebarContainer $collapsed={collapsed}>
      <Header>
        <LogoWrap>
          ...
          <LogoText $hidden={collapsed}>PersonalDB</LogoText>
        </LogoWrap>
